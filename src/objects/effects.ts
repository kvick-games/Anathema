...
import { EntityManager, SceneGameObject } from "../core/gameObject.js";

export interface TimedEffectOptions {
  mesh: any;
  life: number;
  maxLife?: number;
  velocity?: any;
...
export class TimedEffect extends SceneGameObject {
  ...
  constructor(options: TimedEffectOptions) {
    super("effect", options.mesh);
    Object.assign(this, options);
    this.mesh = options.mesh;
  }

  update(_ctx: any, dt: number): void {
    this.life -= dt;
    if (this.velocity) {
    ...
    if (this.maxLife && this.mesh.scale) {
      ...
      if (this.mesh.material?.opacity !== undefined) this.mesh.material.opacity = fade * 0.95;
    }
  ...
  dispose(ctx: any): void {
    ...
    if (this.disposeGeometry && this.mesh.geometry) this.mesh.geometry.dispose();
    if (this.disposeMaterial && this.mesh.material) this.mesh.material.dispose();
  }
}
...
export class EffectFactory {
  ...
  timed(ctx: any, options: TimedEffectOptions): TimedEffect {
    const effect = new TimedEffect(options);
    ctx.scene.add(effect.mesh);
    return this.entities.addEffect(effect);
  }

  spark(ctx: any, position: any, velocity: any, color: number, size: number, life: number): TimedEffect {
  ...
  burstParticles(ctx: any, position: any, color: number, count = 12, power = 5, size = 0.055, yBias = 1.1): void {
    for (let i = 0; i < count; i += 1) {
      ...
      this.spark(
        ctx,
        position.clone().add(new THREE.Vector3(0, 0.8 + Math.random() * 0.5, 0)),
        velocity,
        color,
  ...
  muzzleBurst(ctx: any, position: any, direction: any, color = 0xffd36a): void {
    for (let i = 0; i < 8; i += 1) {
  ...
  slashArc(ctx: any, position: any, direction: any): void {
  ...
  levelCompleteBurst(ctx: any, position: any): void {
  ...
  pulse(ctx: any, position: any, color: number): TimedEffect {
  ...
  private particleMaterial(color: number, opacity = 0.95) {
