...
import type { EnemyTypeConfig, PhysicsConfig } from "../core/types.js";
...
interface EnemyFactoryDependencies {
  scene: any;
  materials: Record<string, any>;
  physics: PhysicsConfig;
  enemyTypes: Record<string, EnemyTypeConfig>;
  nearestFloorZone: (x: number, z: number) => any;
  isSafeSpawnPoint: (x: number, z: number, radius: number) => boolean;
  findSafePoint: (zone: any, radius: number, preferred?: any) => any;
  terrainHeight: (x: number, z: number) => number;
  makeEnemyName: (typeName: string, x: number, z: number, tier: number) => string;
  attachEnemyModel: (enemy: EnemyEntity) => void;
}

export class EnemyFactory {
  constructor(
    private entities: EntityManager,
    private deps: EnemyFactoryDependencies,
  ) {}

  create(x: number, z: number, typeName = "zealot", tier = 1): EnemyEntity {
...
export function createProceduralEnemyVisual(materials: Record<string, any>, typeName: string, type: EnemyTypeConfig, tier: number) {
  const visual = new THREE.Group();
...
export function randomEnemyMutations(tier: number) {
