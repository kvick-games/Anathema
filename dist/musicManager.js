...
export class ProceduralMusicManager {
    ...
    busses;
    noiseBuffer;
    constructor({ audioContext = null, seed = Date.now(), masterVolume = 0.36, palette = "derelict", } = {}) {
    ...
    stop({ fadeOut = 0.9 } = {}) {
        if (!this.started || !this.context || !this.busses)
            return;
    ...
    generateTrack(palette = this.palette) {
        const random = createSeededRandom((this.seed >>> 0)
    ...
    scheduleStep(step, time) {
        ...
        for (const hit of this.track.drums[index] ?? [])
            this.playDrum(hit, time);
        if (this.track.bass[index])
            this.playBass(this.track.bass[index], time);
        if (this.track.arp[index])
            this.playArp(this.track.arp[index], time);
        if (this.track.lead[index])
            this.playLead(this.track.lead[index], time);
        if (this.track.stabs[index])
            this.playStab(this.track.stabs[index], time);
        if (this.track.texture[index])
            this.playTexture(this.track.texture[index], time);
        if (this.track.impacts[index])
            this.playImpact(this.track.impacts[index], time);
    ...
    generateProgression(palette, random) {
        ...
        for (let i = 0; i < palette.bars; i += 1) {
            ...
            if (random() < palette.chaos * 0.22)
                degree += random() > 0.5 ? 1 : -1;
            degrees.push((degree + palette.mode.length) % palette.mode.length);
        }
        if (palette.danger > 0.78 && random() > 0.45)
            degrees.splice(4, 2, 1, 1);
    ...
    generateDrums(palette, random, totalSteps) {
        ...
        for (let step = 0; step < totalSteps; step += 1) {
            ...
            if (localStep === 0)
                drums[step].push({ name: "crash", accent: bar % 4 === 0 ? 1 : 0.6 });
            ...
            if (finalHalf && random() < palette.danger * 0.55)
                drums[step].push({ name: "kick", accent: 0.74 });
            if (localStep === 4 || localStep === 12)
                drums[step].push({ name: palette.danger > 0.54 ? "clap" : "snare", accent: 1.08 });
            if (finalHalf && localStep % 3 === 1 && random() < palette.chaos)
                drums[step].push({ name: "snare", accent: 0.72 });
            ...
            if (random() < palette.chaos * 0.045)
                drums[step].push({ name: "static", accent: 0.55 + random() * 0.5 });
        }
        return drums;
    }
    generateLead(palette, random, progression, totalSteps) {
        ...
        const motif = Array.from({ length: motifLength }, (_, step) => {
            ...
            if (random() > 0.42 + palette.density * 0.42)
                return null;
    ...
    generateArp(palette, random, progression, totalSteps) {
        ...
        if (palette.density < 0.42)
            return arp;
        const rate = palette.density > 0.75 ? 1 : 2;
        for (let step = 0; step < totalSteps; step += rate) {
            const localStep = step % STEPS_PER_BAR;
            if (localStep % 4 === 0 && random() < 0.34)
                continue;
            if (random() > palette.density * 0.82)
                continue;
            const bar = Math.floor(step / STEPS_PER_BAR);
            const chord = progression[bar % progression.length];
    ...
    generateStabs(palette, random, progression, totalSteps) {
        ...
        for (let step = 0; step < totalSteps; step += 1) {
            ...
            if (!hit || random() > palette.danger * 0.52 + palette.drive * 0.16)
                continue;
            const chord = progression[bar % progression.length];
            stabs[step] = {
    ...
    generateTexture(palette, random, progression, totalSteps) {
        ...
        for (let step = 0; step < totalSteps; step += 1) {
            ...
            if (random() < palette.chaos * 0.018)
                texture[step] = { note: chord.notes[1] + 24, type: "glitch", accent: 0.5 + random() * 0.4 };
        }
        return texture;
    }
    generateImpacts(palette, random, totalSteps) {
        ...
        for (let step = 0; step < totalSteps; step += STEPS_PER_BAR) {
            const bar = Math.floor(step / STEPS_PER_BAR);
            if (bar % 4 === 0)
                impacts[step] = { type: "drop", accent: 0.75 + palette.danger * 0.4 };
            if (bar % 4 === 3 && palette.danger > 0.55)
                impacts[step + 15] = { type: "slam", accent: 0.8 + random() * 0.45 };
        }
        return impacts;
    }
    stepToSeconds(steps) {
    ...
    playBass(event, time) {
        ...
        if (event.accent > 1.05)
            this.playNoiseLayer(time, 0.035, 0.032 * event.accent, 180, this.busses.percussion, "lowpass");
    }
    playLead(event, time) {
        const frequency = midiToFrequency(event.note);
        this.playVoice({
    ...
    playTexture(event, time) {
        if (event.type === "bell")
            this.playBell(event.note, time, event.accent);
        if (event.type === "choir")
            this.playChoir(event.note, time, event.accent);
        if (event.type === "riser")
            this.playRiser(event.note, time, event.accent);
        if (event.type === "glitch")
            this.playGlitch(event.note, time, event.accent);
    }
    ...
    playDrum(hit, time) {
        ...
        if (name === "kick")
            this.playKick(time, accent);
        if (name === "snare")
            this.playSnare(time, accent);
        if (name === "clap")
            this.playClap(time, accent);
        if (name === "hat")
            this.playHat(time, accent);
        if (name === "openHat")
            this.playOpenHat(time, accent);
        if (name === "metal")
            this.playMetal(time, accent);
        if (name === "tom")
            this.playTom(time, accent);
        if (name === "static")
            this.playNoiseLayer(time, 0.11, 0.034 * accent, 2600, this.busses.percussion);
        if (name === "crash")
            this.playCrash(time, accent);
    }
    ...
    playVoice({ frequency, endFrequency = frequency, time, duration, type, volume, bus, attack = 0.012, release = 0.2, detune = 0, vibratoDepth = 0, vibratoRate = 5, filter = null, }) {
    ...
    playNoiseLayer(time, duration, volume, frequency, bus, filterType = "bandpass", endFrequency = frequency) {
        ...
        if (Math.abs(endFrequency - frequency) > 1)
