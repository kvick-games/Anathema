...
export function createHudController() {
    ...
    function toggleCodex(show = elements.codex.hidden, snapshot) {
    ...
    function toggleControls(show = elements.controls.hidden, bindings) {
        elements.controls.hidden = !show;
        if (show)
            elements.codex.hidden = true;
        if (show && document.pointerLockElement)
            document.exitPointerLock();
        if (bindings)
            renderControls(bindings);
    }
    function update(snapshot) {
        ...
        if (!elements.codex.hidden)
            renderCodex(snapshot);
    }
    function updateRadarBlips({ enemies, player, lockTarget, pointerYaw }) {
        const center = 105.5;
        const radius = 72;
        const range = 92;
    ...
    function updateEnemyIndicators({ enemies, player, lockTarget, pointerYaw, camera }) {
