...
function setHudMeter(name, value, max = 100, display = `${Math.floor(value)} / ${max}`) {
    ...
    if (hud.meterValues[name])
        hud.meterValues[name].textContent = display;
}
const keys = new Set();
...
function deckMusicName() {
    if (portalActive)
        return "victory";
    if (enemies.length > 0)
        return player.sanity < 42 || enemies.length > Math.max(4, waveHostileCount * 0.45) ? "combat" : "sanctum";
...
function updateMusic(force = false) {
    ...
    if (!force && stateKey === musicStateKey)
        return;
...
function startMusic() {
    ...
    updateMusic(true);
    if (!musicManager.started)
        musicManager.start().catch(() => { });
}
...
function generateLevel() {
    for (const object of levelObjects.splice(0))
        scene.remove(object);
    colliders.length = 0;
    markCollisionDebugDirty();
    floorZones.length = 0;
...
function updateProjectiles(dt) {
    ...
    for (let i = impacts.length - 1; i >= 0; i -= 1) {
        ...
        if (impact.maxLife && impact.mesh.scale) {
            ...
            if (impact.mesh.material?.opacity !== undefined)
                impact.mesh.material.opacity = fade * 0.95;
        }
        if (impact.life <= 0) {
            scene.remove(impact.mesh);
            if (impact.disposeGeometry && impact.mesh.geometry)
                impact.mesh.geometry.dispose();
            if (impact.disposeMaterial && impact.mesh.material)
...
function updateHud() {
    ...
    if (hud.weaponAmmo)
        hud.weaponAmmo.textContent = `${Math.round(weapon.damage)} / ${Math.round(weapon.projectileSpeed)}`;
    if (hud.weaponDetail)
        hud.weaponDetail.textContent = `${weapon.mode} | Deck ${wave} | Hostiles ${enemies.length}`;
    if (hud.weaponHeat)
        setHudBar(hud.weaponHeat, 100 - player.stamina * 0.55 + (input.left || input.right ? 24 : 0), 100);
    ...
    if (hud.quickKitCount)
        hud.quickKitCount.textContent = String(player.repairKits);
    const combatRows = [
...
function restart() {
    ...
    for (const enemy of enemies.splice(0))
        scene.remove(enemy.group);
    for (const item of impacts.splice(0))
